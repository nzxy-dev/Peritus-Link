...
# Link-P Manual and Help Resources Current Version: 1.3.00 (Last Public Release)
...
**To add new notes**, simply press the ``Add-note`` button located in the bottom-right corner—or simply drag a link into the window (if it doesn't respond as expected, please try again; sometimes detection fails on the first attempt).
Next, add or edit a title, a link, and tags. Tags do not serve as a critical, standalone feature; in fact, only the URLs fulfill a truly practical function. Furthermore, the application does not validate whether or not you have entered a valid URL; it simply interprets the input as text to be copied or sent to your browser for opening, depending on the action you execute via the Toolbar.
...
- Pin Button 
> (This function simply moves the card to the very first position, given that—within this app—pinning is essentially automatic and determined by positional order.)
-Thumbnail
- Share buttom ( Works via Browser ) 

**Continuing with the basic functions:** 
Metadata Search. The various text fields—such as title, description, or tags—which otherwise serve no distinct function, exist specifically for **search** purposes. You can find the search bar in the top-right corner. The search operates **in real-time**: you can enter a search term and watch as the display area—where your cards are shown—filters the results instantly based on your input in the search bar. 

> Remember that tags and descriptions—just like the title—do not possess any advanced functionality or formatting; you are free to organize or write them however you wish.
...
### Search Filters/ Ignore Filters:
They serve to ignore certain patterns in the metadata you wish to search for via your search bar—such as order, capitalization, or lowercase letters, among others.
...
> If you are a developer, you can now find a small plugin in Visual Studio Code that provides support for .peritus files.
